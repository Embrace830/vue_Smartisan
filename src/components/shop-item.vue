      ...
      <div>
          ...
          <div class="params-colors">
              <ul class="colors-list">
                  <li v-for="(item,index) in item.sku_info"><a href="javascript:;" @click="tableData(index)" :title="item.spec_json.show_name"><img :src="item.spec_json.image"></a></li>
              </ul>
          </div>
          <div class="item-btns clearfix">
              <span class="item-gray-btn"><router-link :to="{name: 'Item', query: {itemId:item.sku_info[itemIndex].sku_id}}">查看详情</router-link> </span>
              <span @click="addCarPanelData(item.sku_info[itemIndex])" class="item-blue-btn">加入购物车 </span>
          </div>
